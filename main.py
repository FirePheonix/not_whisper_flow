...
from automation import HotkeyManager, Commander
from ui import AppWindow, TrayIcon
...
class NotWhisperFlowApp:
    ...
    def _process_audio(self, audio: np.ndarray):
        ...
        try:
            ...
            # 4. AgentRunner → tool call list
            current_mode = self.window.current_page
            agent_result = self.agent.parse(raw_text, mode=current_mode)
            ...
            for tc in agent_result.calls:
                self._execute_tool(tc, raw_text, prev_tc=prev_tc)
                prev_tc = tc
    ...
    def _execute_tool(self, tc: ToolCall, raw_text: str, prev_tc: ToolCall = None):
        """
